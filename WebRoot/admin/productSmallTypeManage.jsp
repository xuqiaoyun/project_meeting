...
<script type="text/javascript">
	...
	function openProductSmallTypeAddDialog(){
		...
		url="${pageContext.request.contextPath}/smalltypeapi/add";
	}
	...
	function openProductSmallTypeModifyDialog(){
		...
		url="${pageContext.request.contextPath}/smalltypeapi/update/"+row.id;
	}
	...
	function deleteProductSmallType(){
		...
		$.messager.confirm("系统提示","您确认要删除这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("${pageContext.request.contextPath}/smalltypeapi/delete",{id:ids},function(result){
					if(results){
						if(result.exist){
...
<body style="margin:1px;">
	<table id="dg" title="商品小类管理" class="easyui-datagrid"
	 ...
	 url="${pageContext.request.contextPath}/smalltypeapi/show" fit="true" toolbar="#tb">
	 <thead>
	 	<tr>
	 	...
	 	<form id="fm" method="post">
	 		<table cellspacing="8px">
	 			<tr>
	 				<td>所属大类：</td>
	 				<td colspan="3">
	 					<input class="easyui-combobox" id="bName" name="bigType.id" data-options="panelHeight:'auto',editable:true,valueField:'id',textField:'name',url:'${pageContext.request.contextPath}/bigtypeapi/bt'"/>
	 				</td>
	 			</tr>
	 			<tr>
	 				<td>小类名称：</td>
