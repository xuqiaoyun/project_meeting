...
<script type="text/javascript">
	...
	$(function(){
		$("#bName").combobox({
			onSelect:function(record){
				...
				$("#sName").combobox("reload","${pageContext.request.contextPath}/smalltypeapi/querySmallByBigId?id="+record.id);
	...
	function formatProPic(val,row){
		return "<img width=50 height=50 src='${pageContext.request.contextPath}/images/product/"+val+"'>";
	}
	...
	function openProductAddDialog(){
		...
		url="${pageContext.request.contextPath}/goodsapi/add";
	}
	/*修改数据*/
	function openProductModifyDialog(){
		...
		url="${pageContext.request.contextPath}/goodsapi/update/"+row.id;
	}
	...
	function deleteProduct(){
		...
		$.messager.confirm("系统提示","您确认要删除这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("${pageContext.request.contextPath}/goodsapi/delete",{ids:ids},function(result){
					if(result){
	...
	function setProductHot(){
		...
		$.messager.confirm("系统提示","您确认要设置这<font color=red>"+selectedRows.length+"</font>个商品为热卖吗？",function(r){
			if(r){
				$.post("${pageContext.request.contextPath}/goodsapi/updatehot",{ids:ids},function(result){
					if(result){
	...
	function setProductSpecialPrice(){
		...
		$.messager.confirm("系统提示","您确认要设置这<font color=red>"+selectedRows.length+"</font>个商品为特价吗？",function(r){
			if(r){
				$.post("${pageContext.request.contextPath}/goodsapi/updatespecial",{ids:ids},function(result){
					if(result){
						$.messager.alert("系统提示","已设置成功！");
						$("#dg").datagrid("reload");
					}else{
...
<body style="margin:1px;">
	<table id="dg" title="商品管理" class="easyui-datagrid"
	 ...
	 url="${pageContext.request.contextPath}/goodsapi/show" fit="true" toolbar="#tb">
	 <thead>
	 	<tr>
	 		<th field="cb" checkbox="true" align="center"></th>
	 		<th field="id" width="50" align="center">编号</th>
	 	    <th field="proPic" width="80" align="center" formatter="formatProPic">商品图片</th> 
	  ...
	  closed="true" buttons="#dlg-buttons">
	 	<form id="fm" method="POST" enctype="multipart/form-data" >
	 		<table cellspacing="8px">
	 			...
	 			<tr>
	 				<td>价格：</td>
	 				<td colspan="4"><input type="text" id="price" name="price" class="easyui-validatebox" required="true"/></td>
	 			</tr>
	 			<tr>
	 				<td>库存：</td>
	 				<td colspan="4"><input type="text" id="stock" name="stock" class="easyui-validatebox" required="true"/></td>
	 			</tr>
	 			<tr>
	 				<td>商品图片：</td>
	 				<td colspan="4"><input type="file" id="pP" name="pic" /></td>
	 			</tr>
	 			<tr>
	 				<td>所属大类：</td>
	 				<td colspan="4">
	 					<input class="easyui-combobox" id="bName" name="bigTypeId" data-options="panelHeight:'auto',editable:false,valueField:'id',textField:'name',url:'${pageContext.request.contextPath}/bigtypeapi/bt'"/>
	 				</td>
	 			</tr>
	 			<tr>
	 				<td>所属小类：</td>
	 				<td colspan="4">
	 					<input class="easyui-combobox" id="sName" name="smallTypeId" data-options="panelHeight:'auto',editable:false,valueField:'id',textField:'name',url:'${pageContext.request.contextPath}/smalltypeapi/st'"/>
	 				</td>
	 			</tr>
	 			<tr>
	 				<td valign="top">备注：</td>
